...
function PartImage({ partNumber, color }) {
  ...
  return <img className="part-image" src={src} alt={`${partNumber} ${color}`} onError={handleError} />;
}
    ...
    <div className="modal-backdrop">
      <div className="modal wide-modal modal-shell">
        <div className="modal-body">
          ...
          <form id="import-preview-form" className="stack" onSubmit={(e) => { e.preventDefault(); onConfirm({ name, url }); }}>
        ...
        <div className="modal-actions sticky-actions">
          <button form="import-preview-form" type="submit" className="btn primary" disabled={busy}>
              ...
              <tr key={part.id}>
                ...
                <td>
                  ...
                  <div className="image-links inline-links">
                    <a href={`https://www.bricklink.com/v2/search.page?q=${encodeURIComponent(part.part_number)}#T=P`} target="_blank" rel="noreferrer">BrickLink</a>
                    <span>•</span>
...
export default function App() {
  ...
  async function refreshMocs() {
    try {
      ...
      if (selectedMocId && !data.find((m) => m.id === selectedMocId)) setSelectedMocId(data[0]?.id ?? null);
    } catch (err) {
  ...
  return (
    <div className="page">
      <header className="header">
        <div>
          <h1>LEGO MOC Manager</h1>
          <p className="subtitle">Hosted React version with Supabase + Rebrickable CSV import preview.</p>
        </div>
      ...
      {!showBuyList ? (
        ...
        <div className="panel">
          <h2>Buy List</h2>
          <p className="subtitle">This version adds the CSV importer. The next step is porting the grouped buy-list logic from your local version into the hosted app.</p>
        </div>
      )}
        ...
        <div className="modal-backdrop">
          <div className="modal">
            ...
            <form className="stack" onSubmit={handleSavePart}>
              <label><span>Part number</span><input name="partNumber" defaultValue={editingPart.part_number || ""} /></label>
              <label><span>Color</span><input name="color" defaultValue={editingPart.color || ""} /></label>
              <label><span>Required quantity</span><input name="requiredQty" type="number" min="1" defaultValue={editingPart.required_qty ?? 1} /></label>
              <label><span>Have quantity</span><input name="haveQty" type="number" min="0" defaultValue={editingPart.have_qty ?? 0} /></label>
              <label className="checkbox"><input name="ordered" type="checkbox" defaultChecked={!!editingPart.ordered} /><span>Ordered</span></label>
              <label className="checkbox"><input name="arrived" type="checkbox" defaultChecked={!!editingPart.arrived} /><span>Arrived</span></label>
              <label className="checkbox"><input name="completed" type="checkbox" defaultChecked={!!editingPart.completed} /><span>Completed</span></label>
              <div className="toolbar">
