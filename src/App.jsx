...
function groupBuyRows(allParts, orderedState) {
  const grouped = new Map();
  ...
  rows.forEach((row) => row.lines.sort((a, b) => a.mocName.localeCompare(b.mocName)));
  return rows;
}

function BuyListSection({ title, subtitle, rows, mode, onPatch }) {
  return (
    <div className="panel">
      <h2>{title}</h2>
              ...
              {rows.map((row) => (
                <tr key={`${mode}-${row.partNumber}-${row.color}`}>
                  ...
                  <td className="moc-lines-cell">
                    {row.lines.map((line) => (
                      <div key={line.partId} className="buy-line">
...
export default function App() {
  ...
  async function patchPart(id, patch) {
    try {
      ...
      await refreshParts(selectedMocId);
      await refreshAllParts();
    } catch (err) {
  ...
  const toOrderRows = useMemo(() => groupBuyRows(allParts, "to_order"), [allParts]);
  const orderedRows = useMemo(() => groupBuyRows(allParts, "ordered"), [allParts]);
    ...
    <div className="page">
      ...
      {!showBuyList ? (
        ...
        <div className="content">
          ...
          <BuyListSection
            title="Ordered"
            subtitle="Once ordered, these lines move here. Arrival is still tracked per MOC line."
            rows={orderedRows}
            mode="ordered"
            onPatch={patchPart}
          />
