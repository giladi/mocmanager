...
async function fetchRebrickableFallbackFromProxy(partNumber, color) {
  ...
  return {
    ...
    source: data.source || "rebrickable",
  };
}
...
function getReferenceBadgeLabel(resolvedImage) {
  ...
  if (resolvedImage.source === "brickowl") return "Owl ref";
...
function getReferenceBadgeTitle(resolvedImage, requestedColor) {
  if (!resolvedImage?.isReference) return "";
  if (resolvedImage.source === "brickowl") {
    return `Reference image shown from Brick Owl because an exact ${requestedColor} image was not available from the primary sources.`;
  }
    ...
    <div className="panel">
      ...
      <div className="guide-sections">
        ...
        <div className="guide-section">
          ...
          <p>Part images try BrickLink first. If that fails, the app’s Cloudflare proxy checks Rebrickable for an exact-color image, then a generic part-level reference image, then a blue reference image, and can finally fall back to Brick Owl as a reference source when `REBRICKABLE_API_KEY` and `BRICKOWL_API_TOKEN` are configured in Cloudflare. Reference images are marked in the thumbnail as Part ref, Blue ref, or Owl ref and do not change the stored part color requirement.</p>
        </div>
        <div className="guide-section">
          <strong>Notes</strong>
