    ...
    return (
      <div className="part-image-fallback">
        ...
        <div className="image-links">
          ...
          <span>•</span>
          <a href={rebrickableSearchUrl(partNumber)} target="_blank" rel="noreferrer">Rebrickable</a>
        </div>
      </div>
    );
  }

  return <img className="part-image" src={candidates[index]} alt={`${partNumber} ${color}`} onError={handleError} />;
}
...
function importRebrickableCSV(fileName, text) {
  ...
  if (!parts.length) throw new Error("No valid non-spare parts found in the CSV.");
...
function PartTable({ parts, sortField, sortDir, setSortField, setSortDir, onEdit, onDelete, onPatch }) {
          ...
          {parts.map((part) => {
            ...
            return (
              <tr key={part.id}>
                ...
                <td>
                  <div className="part-number-cell">{part.part_number}</div>
                  <div className="image-links inline-links">
                    <a href={brickLinkSearchUrl(part.part_number)} target="_blank" rel="noreferrer">BrickLink</a>
                    <span>•</span>
                    <a href={brickOwlSearchUrl(part.part_number, part.color)} target="_blank" rel="noreferrer">Brick Owl</a>
                    <span>•</span>
                ...
                <td>
                  <input
                    ...
                    min="0"
                    value={part.have_qty}
                    onChange={(e) => onPatch(part.id, {
                      haveQty: Math.max(0, Number(e.target.value || 0))
                    })}
                  />
                </td>
                <td>{missing}</td>
...
function groupBuyRows(allParts, orderedState, mocFilterId) {
  const grouped = new Map();
  ...
  for (const part of allParts) {
    ...
    row.lines.push({
      ...
      belongsToFilteredMoc
    });
  }

  const rows = [...grouped.values()]
    .filter((row) => !mocFilterId || row.matchesFilter)
    .sort((a, b) => a.partNumber.localeCompare(b.partNumber) || a.color.localeCompare(b.color));

  rows.forEach((row) => row.lines.sort((a, b) => {
    if (a.belongsToFilteredMoc !== b.belongsToFilteredMoc) return a.belongsToFilteredMoc ? -1 : 1;
    return a.mocName.localeCompare(b.mocName);
  }));
...
function BuyListSection({ title, subtitle, rows, mode, mocFilterId, onPatch, onOpenMoc }) {
  return (
    <div className="panel">
      ...
      {!rows.length ? (
        <div className="muted">Nothing here right now.</div>
      ) : (
        <div className="table-wrap">
          <table>
            <thead>
...
export default function App() {
  ...
  const [buyListMocFilter, setBuyListMocFilter] = useState("all");
  ...
  async function refreshParts(mocId) {
    try {
      setParts(await listMocParts(mocId));
    } catch (err) {
  ...
  async function refreshAllParts() {
    try {
      setAllParts(await listAllPartsForUser());
    } catch (err) {
  ...
  async function handleCreateMoc() {
    ...
    try {
      setBusy(true);
      const moc = await createMoc({ name: name.trim(), url: url.trim(), sourceFileName: null, userId: session.user.id });
      await refreshMocs();
  ...
  async function handleImportCsv(file) {
    try {
      setCsvPreview(importRebrickableCSV(file.name, await file.text()));
      setError("");
    } catch (err) {
  ...
  async function patchPart(id, patch) {
    try {
      setBusy(true);
      if (selectedMocId) await updatePart(id, patch);
      else await updatePart(id, patch);
  ...
  const sortedParts = useMemo(() => {
    ...
    items.sort((a, b) => {
      const aMissing = Math.max(a.required_qty - a.have_qty, 0);
      const bMissing = Math.max(b.required_qty - b.have_qty, 0);
      const getVal = (item, missing) => {
        switch (sortField) {
          case "part": return item.part_number;
          case "color": return item.color;
          case "need": return item.required_qty;
          case "have": return item.have_qty;
          case "missing": return missing;
          case "ordered": return item.ordered ? 1 : 0;
          case "arrived": return item.arrived ? 1 : 0;
          case "completed": return item.completed ? 1 : 0;
          default: return item.part_number;
        }
      };
      const av = getVal(a, aMissing);
      const bv = getVal(b, bMissing);
  ...
  const toOrderRows = useMemo(() => groupBuyRows(allParts, "to_order", buyFilterValue), [allParts, buyFilterValue]);
  const orderedRows = useMemo(() => groupBuyRows(allParts, "ordered", buyFilterValue), [allParts, buyFilterValue]);
  ...
  return (
    <div className="page">
      <header className="header">
        <div>
          <h1>LEGO MOC Manager</h1>
          <p className="subtitle">Hosted React version with sorting, to-order workflow, and cleaner grouped buy list.</p>
        </div>
        <div className="toolbar">
      ...
      {!showBuyList ? (
        <div className="layout">
          <aside className="sidebar panel">
            ...
            <div className="stack">
              {mocs.map((moc) => (
                <button key={moc.id} className={`moc-item ${moc.id === selectedMocId ? "selected" : ""}`} onClick={() => setSelectedMocId(moc.id)}>
                ...
                <div className="panel">
                  ...
                  <div className="filters">
                    ...
                    <select value={sortField} onChange={(e) => setSortField(e.target.value)}>
                      <option value="part">Sort: Part</option>
                      <option value="color">Sort: Color</option>
                      <option value="need">Sort: Need</option>
                      <option value="have">Sort: Have</option>
                      <option value="missing">Sort: Missing</option>
                      <option value="ordered">Sort: Ordered</option>
                  ...
                  <div className="section-block"><h3>Active</h3><PartTable parts={activeParts} sortField={sortField} sortDir={sortDir} setSortField={setSortField} setSortDir={setSortDir} onEdit={setEditingPart} onDelete={handleDeletePart} onPatch={patchPart} /></div>
                  <div className="section-block"><h3>Ordered / Arriving</h3><PartTable parts={orderedParts} sortField={sortField} sortDir={sortDir} setSortField={setSortField} setSortDir={setSortDir} onEdit={setEditingPart} onDelete={handleDeletePart} onPatch={patchPart} /></div>
                  <div className="section-block"><h3>Completed</h3><PartTable parts={completedParts} sortField={sortField} sortDir={sortDir} setSortField={setSortField} setSortDir={setSortDir} onEdit={setEditingPart} onDelete={handleDeletePart} onPatch={patchPart} /></div>
                </div>
              </>
            ) : (
        ...
        <div className="content">
          <div className="panel">
            <div className="row-between">
              ...
              <div className="buy-filter">
                <label><span>Filter To Order by MOC</span></label>
                <select value={buyListMocFilter} onChange={(e) => setBuyListMocFilter(e.target.value)}>
                  <option value="all">All MOCs</option>
                  {mocs.map((moc) => <option key={moc.id} value={moc.id}>{moc.name}</option>)}
                </select>
              </div>
            </div>
            <p className="subtitle">
              To Order shows missing quantities not yet ordered. When filtered by one MOC, shared parts still show the extra quantity needed by other MOCs too.
            </p>
          </div>

          <BuyListSection
            title="To Order"
            subtitle="Any missing quantity automatically appears here until you mark it as ordered."
            rows={toOrderRows}
            mode="to_order"
          ...
          <BuyListSection
            title="Ordered"
            subtitle="Once ordered, these lines move here. Arrival is tracked per MOC line, and you can move any line back to To Order."
            rows={orderedRows}
            mode="ordered"
            mocFilterId={buyFilterValue}
            onPatch={patchPart}
