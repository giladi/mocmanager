...
export async function listMocs() {
  const { data, error } = await supabase.from("mocs").select("*").order("created_at", { ascending: false });
  if (error) throw error;
  return data;
}

export async function createMoc({ name, url, sourceFileName, userId }) {
  const { data, error } = await supabase.from("mocs").insert({
    user_id: userId,
    name,
    url: url || null,
...
export async function updateMoc(id, patch) {
  ...
  const { data, error } = await supabase.from("mocs").update(payload).eq("id", id).select().single();
  if (error) throw error;
  return data;
}
...
export async function listMocParts(mocId) {
  const { data, error } = await supabase.from("moc_parts").select("*").eq("moc_id", mocId).order("part_number", { ascending: true });
  if (error) throw error;
  return data;
}

export async function createPart(mocId, part) {
  const { data, error } = await supabase.from("moc_parts").insert({
    moc_id: mocId,
...
export async function updatePart(id, patch) {
  ...
  const { data, error } = await supabase.from("moc_parts").update(payload).eq("id", id).select().single();
  if (error) throw error;
  return data;
}
